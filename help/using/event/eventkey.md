...
When the event is received, the value of the key will allow the system to identify the person associated to the event. Associated to a namespace (see [](../event/eventnamespace.md)), the key can be used to perform queries on the Adobe Experience Platform. See [](../building-journeys/journey-orchestration.md).
